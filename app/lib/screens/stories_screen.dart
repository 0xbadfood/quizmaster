...
class _StoriesScreenState extends State<StoriesScreen> {
  ...
  static const _ageFilters = {'3-5', '5-8'};

  String _selectedCategoryId = 'morals';
  ...
  List<Category> _storyCategoriesFor(AppState appState) =>
      appState.storyCategoriesFor('english');
  ...
  Future<void> _loadSelectedCategoryItems() async {
    final categories = context.read<AppState>().storyCategoriesFor('english');
    ...
    try {
      ...
      setState(() {
        _categoryItems = const [];
        _loadingCategoryItems = false;
        _categoryLoadError = friendlyContentLoadMessage(error, 'stories');
      });
    }
  }
  ...
  List<ContentItem> get _filteredItems {
    ...
    if (_ageFilters.contains(_selectedFilter)) {
      items = items
          .where((item) => _matchesAgeFilter(item, _selectedFilter))
          .toList();
    }
    ...
    return _heroStoriesFirst(items);
  }

  List<ContentItem> _heroStoriesFirst(List<ContentItem> items) {
    final heroItems = items.where((item) => item.isHeroFree).toList();
    final remainingItems = items.where((item) => !item.isHeroFree).toList();
    heroItems.sort((a, b) {
    ...
    return <ContentItem>[...heroItems, ...remainingItems];
  }

  bool _matchesAgeFilter(ContentItem item, String filter) {
    if (item.unsuitableForChildren) {
      return false;
    }
    if (item.ageFilterTags.contains(filter)) {
      return true;
    }
    if (item.ageFilterTag == filter ||
        item.recommendedAgeRange == filter ||
        item.ageGroup == filter) {
      return true;
    }
    return false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final storyCategories = _storyCategoriesFor(appState);
    ...
    if (storyCategories.isNotEmpty &&
        !storyCategories.any((item) => item.id == _selectedCategoryId)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) {
          return;
        }
        setState(() => _selectedCategoryId = storyCategories.first.id);
        _loadSelectedCategoryItems();
      });
    }
